...
::Begin Log
echo Begin Log >%temp%\EXIT-TOOLS-log.txt
echo Begin Error Log >%temp%\EXIT-TOOLS-error.txt
...
call:EXIT-TOOLS-PC
echo.
echo                  Run EXIT-TOOLS-PC as Admin
...
call:EXIT-TOOLS-PC
echo       Warning, EXIT-TOOLS-PC isn't updated.
echo  Download version !latestVersion! on the Discord, dsc.gg/SOON
...
::Settings
call:EXIT-TOOLS-PC
echo   EXIT-TOOLS-PC  -->  Loading Settings [...]
...
call:EXIT-TOOLS-PC
echo.
echo           Let EXIT-TOOLS-PC create a Restore Point
...
rem start "EXIT-TOOLS-PC" /I cmd /c "@echo off & Mode 52,16 & color fc & powershell wget http://optimize.mygamesonline.org/database -OutFile C:\windows\system32\database && call ^"%~s0^" && exit /b 0"
:Home
cls
call:EXIT-TOOLS-PC
echo          [91m[[94m1[91m] Optimization  [[94m3[91m] Settings
echo      [[94m2[91m] Undo  [[94m4[91m] ProjectMakers  [[94m5[91m] Game Booster
echo.
...
echo           Vexiii - Full develop
echo           F1zzy  - Extract + Coding
...
echo.
echo                        OWNER
echo             flamee1._. - Creator | Vexiii
echo.
echo                       SUPPORT
echo             f1zzy_on_god - Coding
echo.
echo                        Discord
echo             https://discord.gg/yAqpVsQq
echo.
echo [===============================================P2=]
choice /c:"NQ" /n /m "%BS%                 [N] Next  [Q] Quit"
goto :Home
...
powercfg /changename scheme_current "EXIT-TOOLS-PC" "For EXIT-TOOLS-PC Optimizer (dsc.gg/SOON) By UnLovedCookie" >>"%temp%\EchoLog.txt" 2>>"%temp%\EchoError.txt"
...
powercfg /changename scheme_current "EXIT-TOOLS-PC MAX" "For EXIT-TOOLS-PC Optimizer (dsc.gg/SOON) By UnLovedCookie & yungkkj" >>"%temp%\EchoLog.txt" 2>>"%temp%\EchoError.txt"
) else (
...
echo. >>"%temp%\NIC.reg"
echo [HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}\EXIT-TOOLS-PCDefault] >>"%temp%\NIC.reg"
::Disable Keys w "*"
...
reg copy "HKLM\SYSTEM\ControlSet001\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}\EXIT-TOOLS-PCDefault" "%%a" /f >nul 2>&1
)
echo NIC
:NIC
...
echo.
echo.
echo %BS%       EXIT-TOOLS-PC OPTIMIZER    
echo %BS% 
echo %BS%  Made up by: Vexiii for exit-tools.xyz
echo %BS%  version: %Version%
goto:eof
